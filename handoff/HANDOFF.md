...
`ver-0.1.1.1-dev`, staged and pending the user's commit approval (not yet committed — see `CHANGELOG.md` for the full entry). `ver-0.1.1.0-dev` is already committed, tagged, and pushed to `dev` (`6d81daf`); `ver-0.1.0.0-dev` before it (`be3fb7f`). This pass is documentation-only: `ref/docs/` went from empty to seven per-script pages plus an index, and a ruling landed that `ref/docs/*.md` pages are dev-only — never reaching `main`, though the folder and its keeper file survive there empty, same as every other referenced-but-empty path. The ruling is encoded in both `luna-core-docs-writer` copies, `CLAUDE.md`, and `README.md`. Full detail in `CHANGELOG.md`'s `ver-0.1.1.1-dev` entry.
